/**
 * @file
 * ENC doctor_cart_form behaviors.
 */
(function ($, Drupal, once, drupalSettings) {

  'use strict';

  Drupal.behaviors.encDoctorCartForm = {
    attach (context, settings) {
      const datepickerid = '.doctor-datepicker-wrapper';
      const alt_field = 'input[data-drupal-selector="edit-date"]';
      const form = $('form[data-drupal-selector="enc-doctor-cart-form"]');
      ...
      doctor_datepickers.forEach((doctor_datepicker) => {
        ...
        if (available_dates.length > 0) {
          ...
          if (min_date != '') {
            // calendar_options.minDate = new Date(min_date);
          }
          // calendar_options.minDate = '';

          const calendar = $(doctor_datepicker).datepicker(calendar_options);

          // filterDays(specialist, selected_service);
        }
